...
function Section({ title, description, children, accent = false, action }) {
  return (
    <section className={`overflow-hidden rounded-lg border bg-white ${accent ? 'border-amber-300' : 'border-slate-200'}`}>
      <div className="flex items-center justify-between gap-4 border-b border-slate-200 px-5 py-4">
        <div><h2 className="text-sm font-semibold text-slate-900">{title}</h2>{description && <p className="mt-0.5 text-xs text-slate-500">{description}</p>}</div>
        {action}
      </div>
      {children}
    </section>
...
function StudentDashboard({ data, onOpenClass, onViewAssignments }) {
  ...
  return (
    ...
    <div className="grid gap-5 lg:grid-cols-[minmax(0,1.35fr)_minmax(300px,0.65fr)]">
      <div className="space-y-5">
        ...
        <Section title="Upcoming deadlines" description="Published work you have not submitted" action={<button type="button" onClick={onViewAssignments} className="text-xs font-semibold text-univ-blue hover:underline">View all assignments</button>}>
          ...
          {data.latestAnnouncements.length === 0 && data.recentMaterials.length === 0 ? (
            ...
            <div className="divide-y divide-slate-100">
              {data.latestAnnouncements.slice(0, 4).map((item) => (
                <button key={`announcement-${item._id}`} type="button" onClick={() => onOpenClass(item.offering)} className="flex w-full items-center justify-between gap-4 px-5 py-3 text-left hover:bg-slate-50">
                  <span className="min-w-0"><span className="block truncate text-sm font-medium text-slate-800">{item.title}</span><span className="mt-0.5 block text-xs text-slate-500">Announcement · {classLabel(item)}</span></span>
                  <ArrowRight className="h-3.5 w-3.5 shrink-0 text-slate-400" />
                </button>
              ))}
              {data.recentMaterials.slice(0, 4).map((item) => (
                <button key={`material-${item._id}`} type="button" onClick={() => onOpenClass(item.offering)} className="flex w-full items-center justify-between gap-4 px-5 py-3 text-left hover:bg-slate-50">
                  <span className="min-w-0"><span className="block truncate text-sm font-medium text-slate-800">{item.title}</span><span className="mt-0.5 block text-xs text-slate-500">Material · {classLabel(item)}</span></span>
                  <ArrowRight className="h-3.5 w-3.5 shrink-0 text-slate-400" />
                </button>
              ))}
            </div>
      ...
      <section>
        <div className="mb-3 flex items-center justify-between"><h2 className="text-base font-semibold text-slate-900">My courses</h2><span className="text-xs text-slate-500">{data.classes.length} courses</span></div>
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{data.classes.map((offering) => <button key={offering._id} type="button" onClick={() => onOpenClass(offering)} className="overflow-hidden rounded-lg border border-slate-200 bg-white text-left hover:border-slate-300"><span className="block h-1 bg-[#0c2a52]" /><span className="block p-4"><span className="block text-xs font-bold text-univ-blue">{offering.subjectCode} · {offering.sectionCode}</span><span className="mt-2 block text-sm font-semibold text-slate-900">{offering.subjectName}</span><span className="mt-1 block text-xs text-slate-500">{offering.instructorName || 'Instructor not assigned'}</span><span className="mt-4 flex items-center justify-between border-t border-slate-100 pt-3 text-xs font-semibold text-univ-blue">Open course <ArrowRight className="h-3.5 w-3.5" /></span></span></button>)}</div>
      </section>
    </div>
  );
}

function StaffDashboard({ data, role, onOpenClass, onViewAssignments }) {
  return (
    <div className="space-y-5">
      <p className="text-sm text-slate-600">
        <strong className="font-semibold text-slate-900">{data.counts.classes}</strong> active classes
        <span className="mx-2 text-slate-300">·</span>
        <strong className="font-semibold text-slate-900">{data.counts.pendingGrading}</strong> waiting for grading
        <span className="mx-2 text-slate-300">·</span>
        <strong className="font-semibold text-slate-900">{data.counts.students}</strong> enrolled students
      </p>
      <div className="grid gap-5 lg:grid-cols-[minmax(0,1.35fr)_minmax(300px,0.65fr)]">
        <Section title="Submissions waiting for grading" description={role === 'admin' ? 'Across all active LMS classes' : 'Oldest submissions appear first'} accent={data.pendingSubmissions.length > 0} action={<button type="button" onClick={onViewAssignments} className="text-xs font-semibold text-univ-blue hover:underline">View all</button>}>
...
export default function LmsDashboard({ role, token, onOpenClass, onViewAssignments }) {
    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-600">{role === 'student' ? 'Deadlines and updates across your courses.' : 'Teaching work across active courses.'}</p>
        </div>
        <button type="button" onClick={loadDashboard} className="inline-flex shrink-0 items-center gap-2 rounded-md border border-slate-200 bg-white px-3 py-2 text-xs font-semibold text-slate-700 hover:bg-slate-50">
          <RefreshCw className="h-3.5 w-3.5" /> Refresh
        </button>
      </div>
      {isLoading ? (
        <div className="rounded-lg border border-slate-200 bg-white p-10 text-center text-sm text-slate-500">Loading dashboard...</div>
      ) : error ? (
        <div className="rounded-lg border border-rose-200 bg-rose-50 p-4 text-sm font-medium text-rose-700">{error}</div>
      ) : data ? (
        role === 'student' ? <StudentDashboard data={data} onOpenClass={onOpenClass} onViewAssignments={onViewAssignments} /> : <StaffDashboard data={data} role={role} onOpenClass={onOpenClass} onViewAssignments={onViewAssignments} />
      ) : (
        <div className="rounded-lg border border-slate-200 bg-white p-10 text-center text-sm text-slate-500">No dashboard data.</div>
      )}
    </div>
  );
}
