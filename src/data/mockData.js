...
function createSecondSemesterSubject({ id, code, name, programId, yearLevel, prerequisites, roomBase, scheduleIndex }) {
  ...
  return {
    ...
    prerequisites,
    sections: [
      { id: `${id}-a`, code: `${prefix}-${yearLevel}2M1`, schedule: { ...morning, room: `Room ${roomBase + (scheduleIndex * 2)}` }, instructor: 'TBA', maxSlots: 40, enrolledCount: 0 },
      { id: `${id}-b`, code: `${prefix}-${yearLevel}2A2`, schedule: { ...afternoon, room: `Room ${roomBase + (scheduleIndex * 2) + 1}` }, instructor: 'TBA', maxSlots: 40, enrolledCount: 0 },
    ],
    fee: 4500,
  };
}
...
export const PAYMENT_METHODS = [
  { id: 'cash', label: 'Walk-in / Cashier', icon: 'Banknote' },
