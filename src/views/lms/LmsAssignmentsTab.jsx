...
export default function LmsAssignmentsTab({ offeringId, canManage, isEnabled, token }) {
  ...
  if (selected) {
    ...
    const cannotSubmit = selected.status !== 'published' || selected.submission?.status === 'graded' || (duePassed && !selected.allowLateSubmissions);
    return (
  ...
  return (
    <div className={`grid gap-5 ${canManage ? 'lg:grid-cols-[minmax(0,1fr)_22rem]' : ''}`}>
      <section className="overflow-hidden rounded-lg border border-slate-200 bg-white">
        <div className="border-b border-slate-200 px-5 py-4"><h2 className="text-sm font-semibold text-slate-900">Assignments</h2><p className="mt-0.5 text-xs text-slate-500">{assignments.length} active or closed</p></div>
      ...
      {canManage && (
        <form onSubmit={createAssignment} className="h-fit rounded-lg border border-slate-200 bg-white p-5">
          <h2 className="text-sm font-semibold text-slate-900">New assignment</h2>
          <fieldset disabled={!isEnabled || isSaving} className="mt-4 space-y-3 disabled:opacity-60"><label className="block text-xs font-semibold text-slate-600">Title<input required value={assignmentDraft.title} onChange={(event) => setAssignmentDraft((current) => ({ ...current, title: event.target.value }))} className="mt-1 w-full rounded-md border border-slate-300 px-3 py-2 text-sm outline-none focus:border-univ-blue focus:ring-2 focus:ring-blue-500/20" /></label><label className="block text-xs font-semibold text-slate-600">Instructions<textarea rows="4" value={assignmentDraft.instructions} onChange={(event) => setAssignmentDraft((current) => ({ ...current, instructions: event.target.value }))} className="mt-1 w-full resize-y rounded-md border border-slate-300 px-3 py-2 text-sm outline-none focus:border-univ-blue focus:ring-2 focus:ring-blue-500/20" /></label><DueDateTimeFields value={assignmentDraft.dueAt} onChange={(dueAt) => setAssignmentDraft((current) => ({ ...current, dueAt }))} /><div><label className="block text-xs font-semibold text-slate-600">Points<input type="number" min="1" max="1000" required value={assignmentDraft.points} onChange={(event) => setAssignmentDraft((current) => ({ ...current, points: event.target.value }))} className="mt-1 w-full rounded-md border border-slate-300 px-3 py-2 text-sm outline-none focus:border-univ-blue" /></label></div><label className="flex items-center gap-2 text-xs font-medium text-slate-600"><input type="checkbox" checked={assignmentDraft.allowLateSubmissions} onChange={(event) => setAssignmentDraft((current) => ({ ...current, allowLateSubmissions: event.target.checked }))} className="rounded border-slate-300 text-univ-blue focus:ring-univ-blue" /> Accept late submissions</label><button type="submit" className="inline-flex w-full items-center justify-center gap-2 rounded-md bg-univ-blue px-3 py-2 text-xs font-semibold text-white hover:bg-blue-700 disabled:opacity-60">{isSaving && <Loader2 className="h-3.5 w-3.5 animate-spin" />} Publish assignment</button></fieldset>
        </form>
      )}
    </div>
  );
