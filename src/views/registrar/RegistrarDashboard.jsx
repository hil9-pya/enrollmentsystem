...
import { Users, ShieldCheck, CheckCircle, GraduationCap } from 'lucide-react';
...
export default function RegistrarDashboard({ students, onNavigate }) {
  const metrics = useMemo(() => {
    // Looking at students who have at least reached the registrar queue
    const relevantStudents = students.filter(s => 
      ['payment_confirmed', 'enrolled'].includes(s.status) || s.paymentStatus === 'paid'
    );

    const totalStudents = relevantStudents.length;
    // Those whose payment is confirmed, waiting for registrar validation
    ...
    // Group by status
    const statusData = [
      { name: 'Pending Validation', value: pending },
      { name: 'Officially Enrolled', value: enrolled },
    ].filter(d => d.value > 0);

    // Enrolled by program
    const programData = PROGRAMS.map(prog => {
      const count = relevantStudents.filter(s => s.programId === prog.id && s.status === 'enrolled').length;
      return { name: prog.id.toUpperCase(), count };
    });

    return { totalStudents, pending, enrolled, statusData, programData };
  }, [students]);
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-200 p-8">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-2">
        <div>
          <h1 className="text-2xl font-extrabold text-univ-navy">Registrar Dashboard</h1>
          <p className="text-xs text-slate-500 mt-1.5 font-medium">Overview of enrollment validations and official student records.</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 bg-white p-6 rounded-2xl border border-slate-200 shadow-premium">
          <h2 className="text-xs font-bold text-univ-navy mb-6 uppercase tracking-wider">Officially Enrolled by Program</h2>
          <div className="h-72">
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-premium flex flex-col justify-between">
          <div>
            <h2 className="text-xs font-bold text-univ-navy mb-6 uppercase tracking-wider">Validation Status</h2>
...
function MetricCard({ title, value, icon, color, onClick }) {
  return (
    <div 
      onClick={onClick}
      className="bg-white p-6 rounded-2xl border border-slate-200 shadow-premium flex flex-col justify-between min-h-[120px] cursor-pointer hover:shadow-premium-lg hover:border-univ-blue/30 transition-all duration-200"
    >
